...
(function() {
  ...
  function isValidUrl(url) {
    try {
      ...
      // 🔒 HARDENED: Only allow HTTPS/WSS protocols (reject http:/ws:)
      ...
      // Reasoning Engines
      // Expected structure: ['', '{version}', 'projects', 'locations', 'reasoningEngines', '{id}:{method}']
      // 🔒 HARDENED: Hostname must be exactly {region}-aiplatform.googleapis.com
      const RE_HOSTNAME = /^[a-z0-9-]+-aiplatform\.googleapis\.com$/;
      if (pathSegments.length === 8 &&
        pathSegments[0] === '' &&
        pathSegments[2] === 'projects' &&
        pathSegments[4] === 'locations' &&
        pathSegments[6] === 'reasoningEngines' && RE_HOSTNAME.test(urlObj.hostname)) {
    ...
    if (normalizedUrl && isValidUrl(normalizedUrl)) {
      ...
      try {
        ...
        const proxyFetchOptions = {
          method: 'POST',
          credentials: 'same-origin', // Send httpOnly session cookie (primary auth)
          headers: {
            'Content-Type': 'application/json',
            // Dev-only fallback: backend ignores this header when NODE_ENV=production
            // 🔒 HARDENED: Read from Vite env var at build time, never hardcode secrets
            'X-App-Proxy': (typeof import.meta !== 'undefined' && import.meta.env?.VITE_PROXY_HEADER) || '',
          },
